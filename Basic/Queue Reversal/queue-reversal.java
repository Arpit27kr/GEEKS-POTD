...
class GfG {
    ...
    public Queue<Integer> rev(Queue<Integer> q) {
        // Creating a stack to store the elements temporarily
        Stack<Integer> stack = new Stack<>();

        // Pushing all elements from the queue to the stack
        while (!q.isEmpty()) {
            stack.push(q.poll());
        }

        // Adding the elements back to the queue in reverse order
        while (!stack.isEmpty()) {
            q.add(stack.pop());
        }
